    ...
    def build(self, reset=False, csim=True, synth=True, cosim=False, validation=False, export=False, fpgasynth=False):
        if 'linux' in sys.platform:
            if self.config.get_config_value('Backend', 'Vivado') in templates.backend_map:
                curr_dir = os.getcwd()
                self.backend.build(dir=curr_dir, prj_config=self.config, reset=reset, csim=csim, synth=synth, cosim=cosim, validation=validation, export=export, fpgasynth=fpgasynth)
            else:
