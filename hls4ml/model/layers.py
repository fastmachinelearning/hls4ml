...
class SeparableConv1D(Layer):
    ...
    def initialize(self):
        ...
        self.add_weights_variable(
            name="depthwise",
            var_name="d{index}",
            quantizer=self.get_attr("depthwise_quantizer"),
        )
        self.add_weights_variable(
            name="pointwise",
            var_name="p{index}",
            quantizer=self.get_attr("pointwise_quantizer"),
        )
...
class Conv2D(Layer):
    ...
    def initialize(self):
        if self.get_attr("data_format") == "channels_last":
            ...
            dims = [
                f"OUT_HEIGHT_{self.index}",
                f"OUT_WIDTH_{self.index}",
                f"N_FILT_{self.index}",
            ]
        else:
            shape = [
                self.attributes["n_filt"],
    ...
    def initialize(self):
        if self.get_attr("data_format") == "channels_last":
            shape = [
                self.attributes["out_height"],
                self.attributes["out_width"],
                self.attributes["n_chan"],
            ]
            dims = [
                f"OUT_HEIGHT_{self.index}",
                f"OUT_WIDTH_{self.index}",
                f"N_CHAN_{self.index}",
            ]
        else:
            shape = [
                self.attributes["n_chan"],
...
class GlobalPooling1D(Layer):
    ...
    def initialize(self):
        ...
        self.set_attr(
            "pool_op",
            self.get_attr("class_name").split("Pooling")[0].replace("Global", ""),
        )
...
class Resize(Layer):
    def initialize(self):
        ...
        if len(inp.shape) == 2:  # 1D -> width + chan
            shape = (
                [self.get_attr("out_width"), self.get_attr("n_chan")]
                if self.get_attr("data_format") == "channels_last"
                else [self.get_attr("n_chan"), self.get_attr("out_width")]
            )
            dims = (
                [f"OUT_WIDTH_{self.index}", f"N_CHAN_{self.index}"]
                if self.get_attr("data_format") == "channels_last"
                else [f"N_CHAN_{self.index}", f"OUT_WIDTH_{self.index}"]
            )
        elif len(inp.shape) == 3:  # 2D -> height + width + chan
            shape = (
                [
                    self.get_attr("out_height"),
                    self.get_attr("out_width"),
                    self.get_attr("n_chan"),
                ]
                if self.get_attr("data_format") == "channels_last"
                else [
                    self.get_attr("n_chan"),
    ...
    def initialize(self):
        ...
        if self.attributes["return_state"]:
            ...
            self.add_output_variable(
                state_shape,
                state_dims,
                out_name=self.outputs[1],
                var_name="layer{index}_h",
                type_name="layer{index}_h_t",
            )
            self.add_output_variable(
                state_shape,
                state_dims,
                out_name=self.outputs[2],
                var_name="layer{index}_c",
                type_name="layer{index}_c_t",
            )
    ...
    def _initialize_transforms(self):
        ...
        if self.ref_impl:
            ...
            self.add_weights_variable(
                ...
                var_name="output_transform_w{index}",
                data=np.ones(1),
            )
...
class GarNetStack(GarNet):
    def _initialize_transforms(self):
        ...
        for il in range(self.attributes["n_sublayers"]):
            ...
            self._add_variable(
                name,
                f"input_transform_{il}_w{{index}}",
                kernel,
                frac_width=10,
                quantize=quantize,
            )
            ...
            self._add_variable(
                name,
                f"input_transform_{il}_b{{index}}",
                bias,
                frac_width=10,
                quantize=quantize,
            )
...
class SymbolicExpression(Layer):
    ...
    def initialize(self):
        self.set_attr(
            "expr_t",
            NamedType(*reversed(self.model.config.get_precision(self, "expr"))),
        )
        self.add_output_variable(
            [len(self.get_attr("expression"))],
            [f"N_OUTPUTS_{self.index}"],
            var_name="y",
        )
