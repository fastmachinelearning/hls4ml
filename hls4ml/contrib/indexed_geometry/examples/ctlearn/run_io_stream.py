"""
run_io_stream.py (ctlearn)
=============================
End-to-end conversion, validation and (optional) synthesis of the full
two-block CTLearn-style 2D model, targeting the Xilinx XCKU115.
...
Only io_stream is supported for this model. io_parallel does not scale
to this pixel count (163): the fully-unrolled io_parallel kernels do
not complete C synthesis at this scale (this was confirmed during the
original development of this backbone, on the xcku040/ single-conv
model one directory down, which already showed the same limitation
with a much smaller pipeline -- see that example's README).
...
# CLI
# ---------------------------------------------------------------
parser = argparse.ArgumentParser(description='io_stream integration test for the full ctlearn two-block model')
parser.add_argument(
    '--synth',
    action='store_true',
...
parser.add_argument(
    ...
    'n_features_in for each conv block (20 for the first, 8 for the '
    'second) -- e.g. 1, 2, 4, 5, 10, 20.',
)
parser.add_argument(
    '--layer-rf',
    action='append',
...
parser.add_argument(
    ...
    action='store_true',
    dest='pack_neighbors',
    help='Enable pack_neighbors=True on every NeighborGatherLayer and its '
    'direct consumer (each IndexedConvolutionLayer/IndexedPoolingLayer), '
    'for a single wide stream packet per pixel instead of one packet '
    'per neighbor. Reduces gather latency from ~n_pixels*n_neighbors '
    'to ~n_pixels cycles, for all three gather stages.',
)
parser.add_argument(
    '--fifo-opt',
    action='store_true',
...
from hls4ml.contrib.indexed_geometry import (  # noqa: E402
...
OUTPUT_DIR = os.path.join(HERE, 'hls4mlprj_ctlearn_stream')
...
x = NeighborGatherLayer(neighbor_indices, use_3d_conv=False, name='neighbor_gather_layer_1')(x)
x = IndexedPoolingLayer(use_3d_conv=False, pooling_type='max', temporal_pool_size=1, name='SingleCNNIndexed_block_pool_1')(x)
x = tf.keras.layers.BatchNormalization(momentum=0.99, name='batch_normalization_1')(x)
x = NeighborGatherLayer(neighbor_indices, use_3d_conv=False, name='neighbor_gather_layer_2')(x)
x = IndexedConvolutionLayer(use_3d_conv=False, temporal_kernel_size=1, filters=16, name='SingleCNNIndexed_block_conv_2_1')(x)
...
# Packed gather output: each NeighborGatherLayer and its direct consumer
...
if args.pack_neighbors:
    _packed_pairs = [
        ('neighbor_gather_layer', 'SingleCNNIndexed_block_conv_1_1'),
        ('neighbor_gather_layer_1', 'SingleCNNIndexed_block_pool_1'),
        ('neighbor_gather_layer_2', 'SingleCNNIndexed_block_conv_2_1'),
    ]
    for gather_name, consumer_name in _packed_pairs:
        hls_config['LayerName'][gather_name]['PackNeighbors'] = True
        hls_config['LayerName'][consumer_name]['PackNeighbors'] = True
        print(f'  Override: {gather_name} + {consumer_name} -> PackNeighbors=True')
...
# io_stream overloads were used correctly for the full, real-scale model
...
if ok_diff and ok_agreement:
    print(f'  {PASS} -- CTLearn io_stream integration test completed')
    print(f'  HLS project: {OUTPUT_DIR}')
else:
    print(f'  {FAIL} -- Review metrics above before proceeding')
    raise AssertionError('run_io_stream (ctlearn) failed (csim)')
print('=' * 65)
