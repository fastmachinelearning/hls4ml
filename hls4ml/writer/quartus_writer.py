...
class QuartusWriter(Writer):
                ...
                for layer in model.get_layers():
                    ...
                    for var in vars:
                        ...
                        if var in model_outputs:
                            name = var.cppname
                            newline += '    ' + 'hls_register output_data ' + name + ';\n'
                            var.name += '.data'
    ...
    def write_test_bench(self, model):
        ...
        for line in f.readlines():
            ...
            if 'myproject' in line:
                ...
                newline = line
                newline += indent + 'for(int i = 0; i < {}; i++) {{\n'.format(outvar.size_cpp())
                newline += indent + '  fout << outputs[j].{}[i].to_double() << " ";\n'.format(outvar.member_name)
                newline += indent + '}\n'
                ...
                newline = line
                newline += indent + 'for(int i = 0; i < {}; i++) {{\n'.format(outvar.size_cpp())
                newline += indent + '  std::cout << outputs[j].{}[i].to_double() << " ";\n'.format(outvar.member_name)
                newline += indent + '}\n'
                newline += indent + 'std::cout << std::endl;\n'
            else:
                newline = line
    ...
    def __write_sigmoid_table(self, model, path):
        ...
        table_size = None
        for layer in model.get_layers():
            if ((layer.get_attr('activation') == 'sigmoid') or (layer.get_attr('recurrent_activation') == 'sigmoid')) and (layer.get_attr('table_size') is not None):
