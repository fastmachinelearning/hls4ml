...
def generate_mask_fn(
    ...
    masks = []
    to_fixed = to_acfixed if backend.lower() == 'oneapi' else to_apfixed
    for idx, (k, b, i) in enumerate(zip(Ks, Bs, Is)):
        ...
        masks.append(f'    {fn}')
    body = '\n'.join(masks)
    arguments = 'input_t *inp, output_t *out' if backend.lower() != 'oneapi' else 'input_t &inp, output_t &out'
    mask_fn = f"""
template<typename input_t, typename output_t>
void {name}({arguments}) {{
    {'#pragma HLS INLINE' if backend.lower() != 'oneapi' else ''}
