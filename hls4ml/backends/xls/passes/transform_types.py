...
from hls4ml.backends.xls.xls_types import (
    XLSArrayVariableConverter,
    XLSInplaceArrayVariableConverter,
)
...
class TransformTypes(GlobalOptimizerPass):
    def __init__(self):
        ...
        self.array_var_converter = XLSArrayVariableConverter(type_converter=self.type_converter)
        self.inplace_array_var_converter = XLSInplaceArrayVariableConverter(type_converter=self.type_converter)
    ...
    def transform(self, model, node):
        ...
        for out_name, var in node.variables.items():
            if io_type == 'io_stream':
                raise Exception(f'Not implemented yet IOType {io_type} in {node.name} ({node.__class__.__name__})')
