...
class Conv1DConfigTemplate(LayerConfigTemplate):
    ...
    def format(self, node):
        ...
        params['instructions'] = ','.join(f'ap_uint<config{index_token}::filt_width>({v})' for v in instr_list)
...
class Conv2DConfigTemplate(LayerConfigTemplate):
    ...
    def format(self, node):
        ...
        params['instructions'] = ','.join(
            f'ap_uint<config{index_token}::filt_height * config{index_token}::filt_width>({v})' for v in instr_list
        )
