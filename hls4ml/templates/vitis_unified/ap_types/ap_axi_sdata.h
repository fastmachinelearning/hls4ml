// Copied from https://github.com/Xilinx/hls-utilities (Apache License 2.0)
// simulation_headers/include/ap_axi_sdata.h @ commit 67a366e
...
//#include "ap_fixed.h"
template <int _AP_W, int _AP_I, ap_q_mode _AP_Q, ap_o_mode _AP_O, int _AP_N> struct ap_fixed;
template <int _AP_W, int _AP_I, ap_q_mode _AP_Q, ap_o_mode _AP_O, int _AP_N> struct ap_ufixed;
...
template <typename T> constexpr std::size_t bytewidth = (bitwidth<T> + CHAR_BIT - 1) / CHAR_BIT;
...
// clang-format on
template <typename T, std::size_t WUser = 0, std::size_t WId = 0, std::size_t WDest = 0,
          uint8_t EnableSignals = (AXIS_ENABLE_KEEP | AXIS_ENABLE_LAST | AXIS_ENABLE_STRB), bool StrictEnablement = false>
struct axis {
    static_assert((EnableSignals & 0b10000000) == 0, "Template parameter 'EnableSignals' is invalid only "
                                                     "low 7 bits can be set!");
    friend class stream<axis<T, WUser, WId, WDest, EnableSignals, StrictEnablement>>;
    ...
    static_assert(has_data || has_user || has_id || has_dest || has_keep || has_strb || has_last,
                  "No axis signals are enabled");

    static_assert(StrictEnablement ? has_data == (bool)(EnableSignals & AXIS_ENABLE_DATA) : true,
                  "Found mismatched enablement for DATA signal");
    static_assert(StrictEnablement ? has_user == (bool)(EnableSignals & AXIS_ENABLE_USER) : true,
                  "Found mismatched enablement for USER signal");
    static_assert(StrictEnablement ? has_id == (bool)(EnableSignals & AXIS_ENABLE_ID) : true,
                  "Found mismatched enablement for ID signal");
    static_assert(StrictEnablement ? has_dest == (bool)(EnableSignals & AXIS_ENABLE_DEST) : true,
                  "Found mismatched enablement for DEST signal");

    typedef typename std::conditional<has_data, T, axis_disabled_signal>::type Type_data;
    Type_data data;
...
#define _AXIS_CHANNEL_API(CHAN_NAME)                                                                                        \
    typedef typename std::conditional<has_##CHAN_NAME, ap_uint<width_##CHAN_NAME>, axis_disabled_signal>::type              \
        Type_##CHAN_NAME;                                                                                                   \
    Type_##CHAN_NAME CHAN_NAME;                                                                                             \
    __attribute__((nodebug)) __attribute__((always_inline)) Type_##CHAN_NAME get_##CHAN_NAME() const {                      \
        assert(has_##CHAN_NAME);                                                                                            \
        return CHAN_NAME;                                                                                                   \
    }                                                                                                                       \
    __attribute__((nodebug)) __attribute__((always_inline)) void set_##CHAN_NAME(Type_##CHAN_NAME value) {                  \
        assert(has_##CHAN_NAME);                                                                                            \
        CHAN_NAME = value;                                                                                                  \
    }
...
#define _AXIS_CHANNEL_INTERNAL_API(CHAN_NAME)                                                                               \
    __attribute__((nodebug)) __attribute__((always_inline)) Type_##CHAN_NAME *get_##CHAN_NAME##_ptr() {                     \
...
// clang-format on
template <typename TData, uint8_t EnableSignals = AXIS_ENABLE_DATA, std::size_t WUser = 0, std::size_t WId = 0,
          std::size_t WDest = 0, bool StrictEnablement = true>
using axis_data = axis<TData, WUser, WId, WDest, (EnableSignals | AXIS_ENABLE_DATA), StrictEnablement>;
...
template <std::size_t WUser, uint8_t EnableSignals = AXIS_ENABLE_USER, std::size_t WId = 0, std::size_t WDest = 0,
          bool StrictEnablement = true>
using axis_user = axis<void, WUser, WId, WDest, (EnableSignals & AXIS_DISABLE_DATA), StrictEnablement>;

} // namespace hls

template <std::size_t WData, std::size_t WUser, std::size_t WId, std::size_t WDest,
          uint8_t EnableSignals = (AXIS_ENABLE_KEEP | AXIS_ENABLE_LAST | AXIS_ENABLE_STRB), bool StrictEnablement = false>
using ap_axis = hls::axis<ap_int<WData>, WUser, WId, WDest, EnableSignals, StrictEnablement>;

template <std::size_t WData, std::size_t WUser, std::size_t WId, std::size_t WDest,
          uint8_t EnableSignals = (AXIS_ENABLE_KEEP | AXIS_ENABLE_LAST | AXIS_ENABLE_STRB), bool StrictEnablement = false>
using ap_axiu = hls::axis<ap_uint<WData>, WUser, WId, WDest, EnableSignals, StrictEnablement>;
...
template <typename T, std::size_t WUser, std::size_t WId, std::size_t WDest, uint8_t EnableSignals, bool StrictEnablement>
class stream<axis<T, WUser, WId, WDest, EnableSignals, StrictEnablement>> final {
    typedef axis<T, WUser, WId, WDest, EnableSignals, StrictEnablement> __STREAM_T__;
  ...
  public:
    ...
    AP_NODEBUG bool empty() {
        #pragma HLS inline
        bool tmp = __fpga_axis_valid(V.get_data_ptr(), V.get_keep_ptr(), V.get_strb_ptr(), V.get_user_ptr(),
    ...
    AP_NODEBUG bool full() {
        #pragma HLS inline
        bool tmp = __fpga_axis_ready(V.get_data_ptr(), V.get_keep_ptr(), V.get_strb_ptr(), V.get_user_ptr(),
                                     V.get_last_ptr(), V.get_id_ptr(), V.get_dest_ptr());
        return !tmp;
    }
    ...
    AP_NODEBUG void read(__STREAM_T__ &dout) {
        #pragma HLS inline
        __STREAM_T__ tmp;
        __fpga_axis_pop(V.get_data_ptr(), V.get_keep_ptr(), V.get_strb_ptr(), V.get_user_ptr(), V.get_last_ptr(),
                        V.get_id_ptr(), V.get_dest_ptr(), tmp.get_data_ptr(), tmp.get_keep_ptr(), tmp.get_strb_ptr(),
                        tmp.get_user_ptr(), tmp.get_last_ptr(), tmp.get_id_ptr(), tmp.get_dest_ptr());
        dout = tmp;
    }
    ...
    AP_NODEBUG void write(const __STREAM_T__ &din) {
        ...
        __fpga_axis_push(V.get_data_ptr(), V.get_keep_ptr(), V.get_strb_ptr(), V.get_user_ptr(), V.get_last_ptr(),
                         V.get_id_ptr(), V.get_dest_ptr(), tmp.get_data_ptr(), tmp.get_keep_ptr(), tmp.get_strb_ptr(),
                         tmp.get_user_ptr(), tmp.get_last_ptr(), tmp.get_id_ptr(), tmp.get_dest_ptr());
    }
    ...
    AP_NODEBUG bool read_nb(__STREAM_T__ &dout) {
        #pragma HLS inline
        __STREAM_T__ tmp;
        if (__fpga_axis_nb_pop(V.get_data_ptr(), V.get_keep_ptr(), V.get_strb_ptr(), V.get_user_ptr(), V.get_last_ptr(),
                               V.get_id_ptr(), V.get_dest_ptr(), tmp.get_data_ptr(), tmp.get_keep_ptr(), tmp.get_strb_ptr(),
                               tmp.get_user_ptr(), tmp.get_last_ptr(), tmp.get_id_ptr(), tmp.get_dest_ptr())) {
            dout = tmp;
            return true;
    ...
    AP_NODEBUG bool write_nb(const __STREAM_T__ &in) {
        ...
        __STREAM_T__ tmp = in;
        bool full_n =
            __fpga_axis_nb_push(V.get_data_ptr(), V.get_keep_ptr(), V.get_strb_ptr(), V.get_user_ptr(), V.get_last_ptr(),
                                V.get_id_ptr(), V.get_dest_ptr(), tmp.get_data_ptr(), tmp.get_keep_ptr(), tmp.get_strb_ptr(),
                                tmp.get_user_ptr(), tmp.get_last_ptr(), tmp.get_id_ptr(), tmp.get_dest_ptr());
        return full_n;
    }
