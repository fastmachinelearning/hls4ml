...
void enqueue_layerStream2layer(hls::stream<INPUT_LAYER_STREAM>& src_dma_stream, hls::stream<INPUT_LAYER_ARR>& raw_stream, bool& isLastIndicate){

    INPUT_LAYER_STREAM input_layer_stream_tmp;
    for (unsigned i = 0; i < (SIZE/INPUT_LAYER_ARR::size); i++){
        INPUT_LAYER_ARR ctype;
        src_dma_stream.read(input_layer_stream_tmp);
...
void dequeue_layer2layer(hls::stream<OUTPUT_LAYER_STREAM>& des_dma_stream, hls::stream<OUTPUT_LAYER_ARR>& raw_stream, bool& isLastIndicate){
    ...
    for (unsigned i = 0; i < (SIZE/OUTPUT_LAYER_ARR::size); i++){
        ...
        des_dma_stream.write(output_layer_stream_tmp);
    }

}

void myproject_axi(
    // hls-fpga-machine-learning insert multiIo
