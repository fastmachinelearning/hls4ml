...
void compute_scaled_indices(
    ...
    ComputeIndex: for (unsigned p = 0; p < data_T::size / CONFIG_T::n_chan; p++) {
        ...
        unsigned sw_idx = scale_index<CONFIG_T::filt_width, CONFIG_T::stride, CONFIG_T::n_in>(wp_idx + p);
...
void compute_output(
    ...
    MultLoop: for (unsigned p = 0; p < data_T::size / CONFIG_T::n_chan; p++) {
        ...
        CopyDataFilt: for (unsigned f = 0; f < CONFIG_T::filt_width; f++) {
            #pragma HLS UNROLL
        ...
        if (pixel_idx[p][CONFIG_T::filt_width - 1]) {
    ...
    typename CONFIG_T::bias_t   biases[CONFIG_T::n_filt])
{
    ...
    assert(CONFIG_T::stride <= CONFIG_T::filt_width);

    hls::stream<typename data_T::value_type> data_window[CONFIG_T::filt_width * CONFIG_T::n_chan];
    const int win_depth = CONFIG_T::n_out;
    ...
    ReadInputWidth: for (unsigned i_iw = 0; i_iw < CONFIG_T::n_in / (data_T::size / CONFIG_T::n_chan); i_iw++) {
        #pragma HLS LOOP_FLATTEN
        if (CONFIG_T::strategy == nnet::latency && data_T::size / CONFIG_T::n_chan == 1) {
            #pragma HLS PIPELINE II=CONFIG_T::reuse_factor
        }
        compute_scaled_indices<data_T, CONFIG_T>(i_iw, pixel_idx);
