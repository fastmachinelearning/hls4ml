...
void compute_pool_indices(
    ...
    ComputeIndex: for (unsigned p = 0; p < CONFIG_T::in_pack_factor; p++) {
        ...
        filt_mask[pool_table_height[h_idx] * CONFIG_T::pool_width + pool_table_width[w_idx * CONFIG_T::in_pack_factor + p]] = 1;
    ...
    PixelLoop: for (unsigned p = 0; p < CONFIG_T::in_pack_factor; p++) {
        ...
        if (pixel_idx[p][CONFIG_T::pool_height * CONFIG_T::pool_width - 1]) {
            FiltLoop: for(unsigned c = 0; c < CONFIG_T::n_filt; c++) {
                ...
                if (CONFIG_T::out_pack_factor == 1) { // Saves resources if we don't pack output, compiler will remove the else branch
