...
def main():
    ...
    x = np.random.rand(1, 600, 16)
    coords = np.random.rand(1, 600, 6)
    combined_shift = np.random.rand(1, 600, 6)

    result_pyg = model(torch.Tensor(x), torch.Tensor(coords), torch.Tensor(combined_shift)).detach().numpy()
    print (result_pyg)

    print (config)
    config['Model']['par_factor'] = 5
    print (config)
