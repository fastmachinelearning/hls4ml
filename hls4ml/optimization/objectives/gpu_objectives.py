...
class GPUFLOPEstimator(ObjectiveEstimator):
    ...
    def is_layer_optimizable(self, layer_attributes):
        if not layer_attributes.weight_shape:
            ...
            return True, OptimizationAttributes(structure_type=SUPPORTED_STRUCTURES.STRUCTURED, pruning=True, weight_sharing=False)
    ...
    def layer_resources(self, layer_attributes):
        if not layer_attributes.weight_shape:
            ...
            if 'Dense' in layer_attributes.layer_type.__name__:
                ...
                return [2 * np.prod(layer_attributes.weight_shape) * layer_attributes.output_shape[0] * layer_attributes.output_shape[1] + layer_attributes.weight_shape[3]]
            else:
    ...
    def layer_savings(self, layer_attributes):
        ...
        if weight_sharing:
            logging.warn('Weight sharing does not decrease FLOPs. \
                         It is recommened to use the default attributes, returned from is_layer_optimizable(...)')
            return [0]
        
        if not pruning:
            logging.warn('Pruning needs to be enabled to decrease FLOPs. \
                         It is recommened to use the default attributes, returned from is_layer_optimizable(...)')
            return [0]
        ...
        if structure_type == SUPPORTED_STRUCTURES.STRUCTURED:
            if 'Dense' in layer_attributes.layer_type.__name__:
                ...
                return [2 * np.prod(layer_attributes.weight_shape[0:3]) * layer_attributes.output_shape[0] * layer_attributes.output_shape[1] + 1]
            else:
                raise Exception('Unknown layer encountered when estimating FLOP savings.')
        else:
            logging.warn('FLOP savings occur with structured pruning. \
                         It is recommened to use the default attributes, returned from is_layer_optimizable(...)')
            return [0]   