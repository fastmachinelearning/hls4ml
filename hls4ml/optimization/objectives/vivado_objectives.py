    ...
    def is_layer_optimizable(self, layer_attributes):
        ...
        if layer_attributes.args['hls4ml_attributes'].strategy.lower() == 'resource' and layer_attributes.args['hls4ml_attributes'].reuse_factor > 1 :
            if 36 % layer_attributes.args['hls4ml_attributes'].weight_precision.width == 0:
                ...
                consecutive_patterns = int(math.ceil(2 * 36 / layer_attributes.args['hls4ml_attributes'].weight_precision.width))
            
            return True, OptimizationAttributes(
                SUPPORTED_STRUCTURES.PATTERN, pruning = True, weight_sharing = False,
                    pattern_offset=int(np.prod(layer_attributes.weight_shape) // layer_attributes.args['hls4ml_attributes'].reuse_factor), 
        ...
        if layer_attributes.args['hls4ml_attributes'].strategy.lower() == 'latency':
            ...
            if layer_attributes.args['hls4ml_attributes'].strategy.lower() == 'resource' and layer_attributes.args['hls4ml_attributes'].reuse_factor == 1:
                ...
                if structure_type == SUPPORTED_STRUCTURES.PATTERN:
                    ...
                    if number_of_patterns == layer_attributes.args['hls4ml_attributes'].reuse_factor and saved_one_bram_block:
