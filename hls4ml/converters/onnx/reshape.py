...
def parse_resize_layer(node, input_names, input_shapes, graph):
    ...
    # The followin is used in initialize() method. Probably a better solution would be to have a channels last parameter at QONNX level 
    layer['data_format'] = 'channels_last' if any(node.domain == 'qonnx.custom_op.channels_last' for node in graph.node) else 'channels_first'
