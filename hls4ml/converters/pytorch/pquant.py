...
@pytorch_handler('Quantizer')
def parse_quantizer_layer(operation, layer_name, input_names, input_shapes, node, class_object, data_reader, config):
    assert 'Quantizer' in operation

    q_params = class_object._parameters

    shape = tuple(input_shapes[0][1:])  # type: ignore

    if any([s is None for s in shape]):
        raise ValueError(f'Tensor {input_names[0]} has at least one dimension with no fixed size')
    ...
    overflow_mode: str = class_object.overflow
    round_mode: str = class_object.round_mode
...
def add_quantizer_info(class_object, input_names, input_shapes, output_shape, layer):
    if getattr(class_object, 'quantize_input', False) and hasattr(class_object, 'input_quantizer'):
        if isinstance(class_object.input_quantizer, Iterable):
            iq_confs = [
                parse_quantizer_layer('Quantizer', f'{layer["name"]}_iq_{i}', [input], [shape], None, q, None, None)[0]
            ...
            iq_confs = [
                parse_quantizer_layer(
                    'Quantizer',
                    f'{layer["name"]}_iq',
                    input_names,
                    input_shapes,
                    None,
                    class_object.input_quantizer,
                    None,
                    None,
                )[0]
            ]
    ...
    if getattr(class_object, 'quantize_output', False) and hasattr(class_object, 'output_quantizer'):
        if isinstance(class_object.output_quantizer, Iterable):
            oq_confs = [
                parse_quantizer_layer(
                    'Quantizer', f'{layer["name"]}_oq_{i}', [layer['name']], [output_shape], None, q, None, None
                )[0]
                for q, i in zip(class_object.output_quantizer, [k for k in range(len(class_object.output_quantizer))])
            ]
            oq_shapes = [output_shape for _ in range(len(class_object.output_quantizer))]
        else:
            oq_confs = [
                parse_quantizer_layer(
                    'Quantizer',
