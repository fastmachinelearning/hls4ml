...
* Altera HLS IP Gen and recent Quartus for Intel/Altera FPGAs using the ``Altera`` backend.
* Intel oneAPI 2024.1 to 2025.0 with the FPGA compiler and recent Intel/Altera Quartus using the ``oneAPI`` backend. Newer versions of oneAPI removed FPGA support and **will not work** with ``hls4ml``.
