...
//    std::cout << "    Input feature map size = " << in.size() << " Output predictions size = " << pr.size() << std::endl;
	...
	{
	 int x = ((ac_fixed<16, 6, true>)pr[i] > 1) ? 1 : ((ac_fixed<16, 6, true>)pr[i] <= -1) ? -1 : 0;
	 if(x != layer3_out[i][0].to_double())
	 {
	  std::cout << "FAILURE" << std::endl; 
	  std::cout << "Expected: " << x << " Actual: " << layer3_out[i][0].to_double() << std::endl;
	 }
	}
