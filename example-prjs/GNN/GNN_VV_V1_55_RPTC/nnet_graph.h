...
namespace nnet {
		   ...
		   res_T     B[CONFIG_T::n_edge][2*CONFIG_T::n_input_dim])
  {
    ...
    if (CONFIG_T::io_type == io_parallel){
    	...
    	#pragma HLS PIPELINE II=CONFIG_T::reuse_factor  //--Commenting
    ...
    else if (CONFIG_T::io_type == io_serial)
    {
    	...
    	// Multiply bi = Ri^T X
    InitMultBi1:
    for(int ii = 0; ii < CONFIG_T::n_edge/2; ii++)
    {
    	#pragma HLS unroll factor=2//-----> New Addition
    ...
    InitMultBi2:
        for(int ii = CONFIG_T::n_edge/2; ii < CONFIG_T::n_edge; ii++)
        {
        	#pragma HLS unroll factor=2//-----> New Addition
        	if (CONFIG_T::io_type == io_serial)
        	{
		   ...
		   res_T     M[CONFIG_T::n_node][3*CONFIG_T::n_input_dim])
  {
    ...
    MultRiX1:
    for(int ii = 0; ii < CONFIG_T::n_edge/2; ii++) {
    	#pragma HLS unroll factor=2//-----> New Addition and commenting the following
    	if (CONFIG_T::io_type == io_serial)
    	{
    		#pragma HLS PIPELINE
    	}
    ...
    // Multiply mi = Rwi bo
    // Multiply mo = Rwo bi
    // int counter = 0; // Did not find any task of this
    MultConcateMi1:
    ...
    MultConcateMi2:
	for(int ii = 12; ii < CONFIG_T::n_node; ii++)
	{
		#pragma HLS unroll factor=2//-----> New Addition
		if (CONFIG_T::io_type == io_serial)
		{
