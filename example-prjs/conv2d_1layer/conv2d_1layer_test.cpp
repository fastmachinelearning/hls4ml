...
#include "firmware/conv2d_1layer.h"
...
CCS_MAIN(int argc, char *argv[])
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      conv2d_1layer(conv2d_input,layer2_out,w2,b2);      
...
//#if 0
    //  if (e % CHECKPOINT == 0) {
	  ...
	  result_t tmp = layer2_out[i];
          ac_fixed<16, 6, true> tmpVal = tmp[0];
	  if(tmp[0].to_double() != pr[i])
	  {
	   std::cout << "Expected: " << pr[i] << " Actual: " << tmp[0].to_double() << std::endl;
	   return 1;
	  }
        }
