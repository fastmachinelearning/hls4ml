...
unsigned get_top_prediction_fxd(result_t *data, unsigned size) {
    float max_value = FLT_MIN;
    unsigned max_index = 0;
    for (unsigned i = 0; i < size; i++) {
        if (data[i].to_double() > max_value) {
            max_value = data[i].to_double();
            max_index = i;
        }
    }
    return max_index;
...
unsigned get_top_prediction(std::vector<float> data, unsigned size) {
    float max_value = FLT_MIN;
    unsigned max_index = 0;
    for (unsigned i = 0; i < size; i++) {
        if (data[i] > max_value) {
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      if (e % CHECKPOINT == 0) {
        ...
        std::cout << std::endl;
        std::cout << "INFO:     Top prediction: " << get_top_prediction(pr, N_LAYER_6) << std::endl;
        ...
        std::cout << std::endl;
        std::cout << "INFO:     Top quantized prediction: " << get_top_prediction_fxd(layer7_out, N_LAYER_6) << std::endl;
      }
    }
    fin.close();
    fpr.close();
    ...
    for(int i = 0; i < N_LAYER_6; i++) {
      std::cout << "INFO: PREDICTIONS: " << layer7_out[i].to_double() << " ";
    }
    std::cout << std::endl;
    std::cout << "INFO: Top prediction: " << get_top_prediction_fxd(layer7_out, N_LAYER_6) << std::endl;
