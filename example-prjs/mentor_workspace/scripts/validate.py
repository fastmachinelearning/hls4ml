...
def plot_histogram(absolute_error, tool):
    ...
    plt.savefig('./validate-' + tool + '.png')
    plt.show()

def main(args):
    ref_data = np.loadtxt(args.ref_file).flatten()
    imp_data = np.loadtxt(args.imp_file).flatten()
    ...
    plot_histogram(absolute_error, args.tool)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', action='store', dest='tool', help='HLS Tool [vivado | catapult].')
