...
set_top mnist_mlp
add_files firmware/mnist_mlp.cpp -cflags "-std=c++0x"
add_files -tb mnist_mlp_test.cpp -cflags "-std=c++0x"
...
if {$opt(synth)} {
  ...
  if {$opt(cosim)} {
    ...
    add_files -tb mnist_mlp_test.cpp -cflags "-std=c++0x -DRTL_SIM"
    set time_start [clock clicks -milliseconds]
