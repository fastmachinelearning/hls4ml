...
#include "firmware/ternary_tanh.h"
...
//    std::cout << "    Input feature map size = " << in.size() << " Output predictions size = " << pr.size() << std::endl;
      ...
      ternary_tanh(conv2d_input,layer3_out,w2,b2);

     	for (int i = 0; i < OUT_HEIGHT_2*OUT_WIDTH_2*N_FILT_2; i++)
	{
	 // NOTE: Times pr[i] by 2, because stream and non-stream examples are different between nnet_activation and nnet_activation_stream.
	 int x = ((ac_fixed<16, 6, true>)(2*pr[i]) > 1) ? 1 : ((ac_fixed<16, 6, true>)(2*pr[i]) <= -1) ? -1 : 0;
	 if(x != layer3_out[i].to_double())
	 {
	  std::cout << "FAILURE" << std::endl; 
	  std::cout << "Expected: " << x << " Actual: " << layer3_out[i].to_double() << std::endl;
	 }
	} 
