...
#include "firmware/conv1d_1layer.h"
  ...
  std::ofstream fout(RESULTS_LOG);
    ...
    if (!loaded_weights) {
        //hls-fpga-machine-learning insert load weights
            nnet::load_weights_from_txt<model_default_t, 15>(w2, "w2.txt");
...
//    std::cout << "    Input feature map size = " << in.size() << " Output predictions size = " << pr.size() << std::endl;
      ...
      conv1d_1layer(input_1,layer2_out,w2,b2);

      for(int i = 0; i < N_OUTPUTS_2; i++)
      {
	if(fabs(pr[i]-layer2_out[i][0].to_double()) > 0.01)
	{
      ...
      nnet::print_result<result_t, N_OUTPUTS_2*N_FILT_2>(layer2_out, fout);
    }
    fin.close();
    fpr.close();
  } else {
