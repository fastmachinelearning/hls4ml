...
#ifndef NNET_DERIVATIVE_H_
#define NNET_DERIVATIVE_H_
...
// *************************************************
template<class data_T, class res_T, typename CONFIG_T>
void  deriv_relu(data_T data[CONFIG_T::n_in], res_T res[CONFIG_T::n_in])
{
    if (CONFIG_T::io_type == io_parallel){
        #pragma HLS PIPELINE
    }
...
// *************************************************
inline float deriv_sigmoid_fcn_float(float input) {
  float sig = 1.0 / (1 + std::exp(-input));
  return sig * (1-sig);
}

template<typename CONFIG_T, int N_TABLE>
void init_deriv_sigmoid_table(typename CONFIG_T::table_t table_out[N_TABLE])
{
    for (int ii = 0; ii < N_TABLE; ii++) {
        // First, convert from table index to X-value (signed 8-bit, range -8 to +8)
        float in_val = 2*8.0*(ii-float(N_TABLE)/2.0)/float(N_TABLE);
        // Next, compute lookup table function
        typename CONFIG_T::table_t real_val = deriv_sigmoid_fcn_float(in_val);
        //std::cout << "Lookup table In Value: " << in_val << " Result: " << real_val << std::endl;
        table_out[ii] = real_val;
    }
}
...
template<class data_T, class res_T, typename CONFIG_T>
void  deriv_sigmoid(data_T data[CONFIG_T::n_in], res_T res[CONFIG_T::n_in])
{
    // Initialize the lookup table
#ifdef __HLS_SYN__
    bool initialized = false;
    typename CONFIG_T::table_t deriv_sigmoid_table[CONFIG_T::table_size];
#else
    static bool initialized = false;
    static typename CONFIG_T::table_t deriv_sigmoid_table[CONFIG_T::table_size];
#endif
    if (!initialized) {
        init_deriv_sigmoid_table<CONFIG_T, CONFIG_T::table_size>(deriv_sigmoid_table);
        initialized = true;
    }
    ...
    for (int ii=0; ii<CONFIG_T::n_in; ii++) {
        ...
        if (index < 0)   index = 0;
        if (index > CONFIG_T::table_size-1) index = CONFIG_T::table_size-1;
        res[ii] = (res_T) deriv_sigmoid_table[index];
    }
}
