    ...
    typename CONFIG_T::bias_t    biases[CONFIG_T::n_out])
{
    ...
    if (CONFIG_T::io_type == io_parallel){
        ...
        int test  = (CONFIG_T::n_in*CONFIG_T::n_out/ float(CONFIG_T::reuse_factor));
        int testb = (CONFIG_T::n_out/ float(CONFIG_T::reuse_factor));
	if(test == 0) test=1;
        #pragma HLS ARRAY_PARTITION variable=biases block factor=testb
        #pragma HLS ARRAY_PARTITION variable=mult block   factor=test
        #pragma HLS ARRAY_PARTITION variable=acc block    factor=testb
 	if(CONFIG_T::reuse_factor != 1) {
          #pragma HLS ARRAY_RESHAPE variable=weights block factor=test
	} 
        #pragma HLS RESOURCE variable=weights core=ROM_2P_BRAM
        #pragma HLS ALLOCATION instances=mul limit=multiplier_limit operation
      
  //int cycle_factor = ceil(float(CONFIG_T::n_in*CONFIG_T::n_out)/float(CONFIG_T::reuse_factor));
  //#pragma HLS ARRAY_PARTITION variable=weights  factor=cycle_factor
		...
		) 
{
  ...
  #pragma HLS ARRAY_PARTITION variable=inputmult complete
  #pragma HLS ARRAY_PARTITION variable=acc       complete
  //#pragma HLS RESOURCE variable=param_W core=ROM_2P_BRAM
  //#pragma HLS ARRAY_RESHAPE variable=param_W  cyclic   factor=nout*2
  //#pragma HLS PIPELINE
		...
		) 
{
 ...
 Prod1: for(int ii = 0; ii < nin; ii++) {
  ...
  for(int ii = 0; ii < nout; ii++) inputacc[ii] = param_b[ii];
 Accum1: for(int ii = 0; ii < nin; ii++) {
  Accum2: for(int jj = 0; jj < nout; jj++) {
      int index = ii*nout + jj;
		...
		) 
{
  ...
  #pragma HLS function_instantiate variable=param_W
  #pragma HLS RESOURCE variable=param_W core=ROM_2P_BRAM
  #pragma HLS ARRAY_PARTITION variable=inputmult complete dim=1
  #pragma HLS ARRAY_PARTITION variable=inputacc  complete 
  #pragma HLS ARRAY_RESHAPE   variable=param_W   factor=2 
