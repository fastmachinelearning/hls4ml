...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    ...
    outputdat layer9_out[num_iterations];
    ...
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      ihc_hls_enqueue(&layer9_out[e], myproject, input_1[e]);
    }
    ihc_hls_component_run_all(myproject);
    for(int j = 0; j < e; j++) {
      //hls-fpga-machine-learning insert tb-output
      for(int i = 0; i < N_LAYER_8; i++) {
        fout << layer9_out[j].data[i] << " ";
      }
      fout << std::endl;
      if (j % CHECKPOINT == 0) {
    ...
    for (int i = 0; i < num_iterations; i++) {
      ...
      ihc_hls_enqueue(&layer9_out[i], myproject, input_1[i]);
    }
    ihc_hls_component_run_all(myproject);
