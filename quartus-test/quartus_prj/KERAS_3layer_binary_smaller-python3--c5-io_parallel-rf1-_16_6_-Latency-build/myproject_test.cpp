...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    ...
    inputdat input1[num_iterations];
    outputdat layer16_out[num_iterations];
    ...
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      std::copy(in_begin, in_end, input1[e].data);
      in_begin = in_end;

      //hls-fpga-machine-learning insert top-level-function
      ihc_hls_enqueue(&layer16_out[e], myproject, input1[e]);
    }
    ihc_hls_component_run_all(myproject);
    for(int j = 0; j < e; j++) {
      //hls-fpga-machine-learning insert tb-output
      for(int i = 0; i < N_LAYER_14; i++) {
        fout << layer16_out[j].data[i] << " ";
      }
      fout << std::endl;
      if (j % CHECKPOINT == 0) {
    ...
    for (int i = 0; i < num_iterations; i++) {
      ...
      std::fill_n(input1[i].data, N_INPUT_1_1, 0.);
      ihc_hls_enqueue(&layer16_out[i], myproject, input1[i]);
    }
    ihc_hls_component_run_all(myproject);
