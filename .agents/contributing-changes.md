...
- Test through whole-model conversion, not by calling a parser handler directly. The dispatch code around
  the handlers has behavior of its own — skip lists, input wiring, splitting of embedded activations — and
  a handler-level test bypasses all of it. Handler-level tests can supplement model-level ones, never
  replace them.
- A test that asserts a conversion error must put the `config_from_*` call inside `pytest.raises` as well:
  the config helpers parse the model themselves, so the expected error can be raised there rather than in
  the convert call.
- Every user-settable option the change adds needs one test that fails if the option is silently ignored:
  assert its observable effect — the file it produces, the name it changes in the generated code, the
  error it must raise — not only that conversion and prediction succeed. An option lost in a `**kwargs`
  sink or an unforwarded parent call passes every numeric test.
- Give invalid configurations negative tests that also pin the stage: a bad option should fail at
  conversion, and the test should break if the failure moves to `compile()` or `build()`.
...
Run the whole test suite before a pull request, not only the tests you added — most of it needs no HLS
tools, since C simulation compiles with an ordinary C++ compiler. Backends whose toolchains are not
installed will fail on environment; that is expected.

The rule for reading the result: **for every backend whose toolchain is installed locally, that backend's
tests must pass completely.** Environmental failures elsewhere are acceptable at a checkpoint, never as the
