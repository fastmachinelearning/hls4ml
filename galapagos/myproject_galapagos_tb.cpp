...
int main()
{
    ...
    galapagos_stream <float>  in;
    galapagos_stream <float>  out;
    ...
    for(int i=0; i<Y_INPUTS; i++){
        for(int j=0; j<N_CHAN; j++){
            std::cout << "BEFORE CAST SENDING " << data_str[i][j] << std::endl;
            gp.data = data_str[i][j];
            std::cout << "AFTER CAST SENDING " << gp.data << std::endl;
            gp.dest = DEST;
