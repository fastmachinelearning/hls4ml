...
# 	'layername': "test",
# 	'outputshape': 2,
# 	'attrlist' : {'Attribute': ['n_filt','epsilon','ops'],'WeightAttribute': ['gamma','beta'],'TypeAttribute': ['gamma','beta']},
...
def pythonWriter(filedir,config):
	...
	# Input output shape missing
			...
			for att in config["attrlist"]['WeightAttribute']:
				newline += dindent + f'{att} = self.model.get_weights_data(self.name, \'{att}\')\n' + dindent + f'self.add_weights_variable(name=\'{att}\', var_name=\'{att}{{index}}\', data={att}) \n'
	...
	# Struct config template
			...
			newline = f'{config["layername"]}_function_template  = (\n' + indent + f'\'nnet::{config["layername"]}<{{data_t}}, {{res_t}}, {{CONFIG_T}}>({{data}}, {{res}} '
