...
def test_parameter_objective():
    ...
    assert([conv_filters * conv_channels * np.prod(conv_shape)] == ParameterEstimator.layer_resources(model_attributes['conv2d']))
    assert([0] == ParameterEstimator.layer_resources(model_attributes['flatten']))
    assert([conv_filters * np.prod(input_shape) * dense_units] == ParameterEstimator.layer_resources(model_attributes['dense']))
