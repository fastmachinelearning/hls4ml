...
def test_dense_unrolled(io_type, reuse_factor, backend):
    input_shape = (16,)
    ...
    config['Model']['Strategy'] = 'Unrolled'
...
# Tests a wide range RF on streaming Conv1D/2D to ensure the unrolled Dense is correct
@pytest.mark.parametrize('dim', [1, 2])
@pytest.mark.parametrize('io_type', ['io_stream'])
@pytest.mark.parametrize('reuse_factor', [1, 3, 9, 27, 54, 108])
def test_dense_unrolled_streaming_conv(dim, io_type, reuse_factor):
    input_shape = (8,) * dim + (3,)
    X = np.random.rand(100, *input_shape)
    ...
    output_dir = str(test_root_path / f'hls4mlprj_dense_unrolled_conv{dim}d_{io_type}_{reuse_factor}')
...
def test_rnn_unrolled(rnn_layer, backend, io_type, static, reuse_factor):
    ...
    hls_config['LayerName'][layer_name]['Strategy'] = 'Unrolled'
    hls_config['LayerName'][layer_name]['ReuseFactor'] = reuse_factor
    prj_name = f'hls4mlprj_rnn_unrolled_{layer_name}_static_{int(static)}_{io_type}_{reuse_factor}_{backend}'
