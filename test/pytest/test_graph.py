...
def test_graph_manipulation(request, parameters, iotype):
...
def test_graph_branch(request, iotype, batch):
    odir = str(test_root_path / get_pytest_case_id(request))
...
@pytest.mark.parametrize('iotype', ['io_parallel', 'io_stream'])
def test_final_reshape(request, iotype):
...
def test_broadcast_stream(request, shapes, layer):
...
def test_multiple_outputs(request, batch):
